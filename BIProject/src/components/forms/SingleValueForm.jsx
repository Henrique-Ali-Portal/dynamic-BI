...
function SingleValueForm({ onSubmit, onCancel, selectedDataSource, selectedRelationship, defaultValues = {} }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <h2 className="text-2xl font-bold mb-6">{isEditing ? "Editar Valor Único" : "Configurar Valor Único"}</h2>
      ...
      {/* Column Selection */}
      <div className="mb-4">
        <label htmlFor="column" className="block text-sm font-medium text-gray-700">Coluna</label>
        <select
          id="column"
          {...register('column', { required: 'Selecione uma coluna' })}
          className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"
        ...
        {errors.column && <p className="mt-2 text-sm text-red-600">{errors.column.message}</p>}
      </div>
